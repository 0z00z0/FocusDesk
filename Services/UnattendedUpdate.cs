...
/// unattended and replaces the files this process holds, so the process that asked for the update is
...
/// <c>{param:UPDATEFROMAPP}</c> and writes <see cref="RefusalFileName"/>. Neither side can read the
/// other's constants, so <c>Tests\UnattendedUpdateTests.cs</c> holds the pair together.</remarks>
internal static class UnattendedUpdate
{
    ...
    /// under a silent run has no wizard behind it and no process left to own it; the version that
    /// starts next reports the outcome instead. <c>/LOG</c> because nothing else records what an
    /// unattended run did.
    /// </summary>
    ...
    /// <summary>Records the attempt as Setup is started, so a version that never gets the chance to
    /// write anything is still accounted for. Never throws.</summary>
